...
class ReplayBuffer:
    ...
    # save current replay_memory deque to file
    def save(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump(self.replay_memory, f)

    # load past replay_memory to deque
...
class Agent:
    ...
    def train(self, start_ep=0, start_step=0, eps_until_save=20, save_checkpoints=True):
        episode = start_ep
        ...
        last_x_rewards = deque(maxlen=eps_until_save)
        ...
        checkpoint_directory = "./checkpoints_run" + run_text
        if not os.path.exists(checkpoint_directory) and save_checkpoints:
            os.mkdir(checkpoint_directory)

        time = str(datetime.datetime.now())
        output_file = "./output_text/output_run" + run_text + "_ep" + str(episode) + ".txt"
        data_file = "./output_text/output_run" + run_text + "_ep" + str(episode) + "_data.txt"
        ...
        while episode < self.max_episodes:
            ...
            last_x_rewards.append(episode_reward)
            last_x_steps_per_episode.append(episode_step)
            mean_episode_reward = np.mean(last_x_rewards)
            mean_steps_per_episode = np.mean(last_x_steps_per_episode)
            
            #log testing results
            with open(data_file, "a") as f:
                f.write(f'{episode} {episode_reward:.4f} {loss:.5f} {epsilon:.3f} {episode_step}\n')
                
            if episode % eps_until_save == 0:
                msg = '\n' + f'Episode {episode} (Step {total_step}) - Moving Avg Reward: {mean_episode_reward:.4f} Loss: {loss:.5f} Epsilon: {epsilon:.3f} Avg Steps Per Episode: {mean_steps_per_episode}\n'
            ...
            # save replay buffer every 200 episodes
            if episode % 200 == 0 and episode != 0:
                self.save_replay_memory("./output_replay_memory/run" + str(run) + "_ep" + str(episode))
                
            #stop training if the mean of the latests rewards is nearing 10
            if mean_episode_reward >= 10:
    ...
    def load_replay_memory(self, filename):
